...
public class LaggCheck {
    ...
    public LaggCheck(String licenseKey, String validationServer, Plugin plugin) {
        this.licenseKey = licenseKey;
        this.plugin = plugin;
        this.validationServer = validationServer;
    }

    public LaggCheck setSecurityKey(String securityKey) {
        this.securityKey = securityKey;
        return this;
    }

    public LaggCheck setConsoleLog(LogType logType) {
        this.logType = logType;
        return this;
    }

    public LaggCheck debug() {
        debug = true;
        return this;
    }

    public boolean register() {
        log(0, "-------------[ Kingdom - License ]-------------");
        log(0, "Checking if the license is correct.");
        ValidationType vt = isValid();
        if (vt == ValidationType.VALID) {
            log(1, "The license key is correct!");
            log(0, "-------------[ Kingdom - License ]-------------");
            return true;
        } else {
            log(1, "License is not correct.");
            log(1, "Failed as a result of " + vt.toString());
            log(1, "The plugin is disabling..");
            log(0, "-------------[ Kingdom - License ]-------------");
